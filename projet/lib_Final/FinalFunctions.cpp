...
void printTunnel(uint32_t nElements, uint16_t *leftDistances,
  ...
  for (int8_t i = TOTAL_LINES; i >= 0; i--) {
    ...
    // Nous faison *10 pour permettre l'arondissement de la valeur
    uint16_t indexDistance =
        (nElements - 1) * i * 10 / TOTAL_LINES;  /// i.e. 199 * 95 / 95

    // On arrondit la valeur puis on divise par 10 pour obtenir le nombre
    // souhaite
...
void smoothingTunnel(uint16_t nElements, uint16_t *distance) {
  ...
  for (uint8_t i = 2; i <= 93; ++i) {
    uint16_t index2 =
        (nElements - 1) * (i - 1) * 10 / 95;          /// i.e. 199 * 95 / 95
    ...
    // On verifie d'abord si -1 et 1 sont a peu pres la meme chose et si oui on
    // fait en sorte que le nombre entre les 2 prennens la meme valeur que ses
    // voisines pour faire des ligne droites
