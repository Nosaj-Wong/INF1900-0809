    ...
    switch (PathPart) {
      ...
      case secondOrFourthTurn:
        ...
        if (turnCounter == 2) {
          ...
          LEFT_CAPTOR_ADJUST = -11;
          RIGHT_CAPTOR_ADJUST = 49;
        }
      ...
      case transmit:
        ...
        lengthLeft = lengthLeft * 254 / 100;
        if (lengthLeft % 10 >= 5) {
          lengthLeft = (lengthLeft + 10);
        }
        lengthLeft/=10;

        // On calcule la langueur du mur droite
        lengthRight = calculateLenghtTest3(rightData, nData);
        lengthRight = lengthRight * 254 / 100;
        if (lengthRight% 10 >= 5) {
          lengthRight = (lengthRight + 10);
        }
        lengthRight/=10;
