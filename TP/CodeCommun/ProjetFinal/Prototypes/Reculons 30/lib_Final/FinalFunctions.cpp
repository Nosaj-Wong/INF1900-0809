...
// if(distances[indexLine - 1] == distances[indexLine + 1]){ smooth la mesure du milieu ... }
...
void smoothingTunnel(uint16_t nElements, uint16_t *distance) {
    ...
    for (uint8_t indexLine = 1; indexLine <= 94; ++indexLine) {
        ...
        if (distance[indexLine - 1] <= distance[indexLine + 1] + 2 &&
            distance[indexLine - 1] >= distance[indexLine + 1] - 2) {
            // Pic vers la gauchek
            // On verifie si -2 et 2 sont a peu pres la meme chose
            if (distance[indexLine - 2] <= distance[indexLine + 2] + 2 &&
                distance[indexLine - 2] >= distance[indexLine + 2] - 2 &&
                // On verifie si -2 est suffissament plus grand que -1
                distance[indexLine - 2] > distance[indexLine - 1] + 4) {
                ...
                distance[indexLine] -= distance[indexLine - 2] - distance[indexLine - 1];
                // Pic vers la droite
                // On verifie si -2 et 2 sont a peu pres la meme chose
            } else if (distance[indexLine - 2] <= distance[indexLine + 2] + 2 &&
                       distance[indexLine - 2] >= distance[indexLine + 2] - 2 &&
                ...
                distance[indexLine] += distance[indexLine - 1] - 4 - distance[indexLine - 2];
            } else {
                // Smooth mur plat
                distance[indexLine] = distance[indexLine - 1];
            }
        }
    }
}
