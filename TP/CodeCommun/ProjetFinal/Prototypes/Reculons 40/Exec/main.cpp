...
ISR(TIMER1_COMPB_vect){
  ...
  read(can,leftData,rightData,nData,LEFT_CAPTOR_ADJUST,RIGHT_CAPTOR_ADJUST);
...
int main() {
  ...
  const uint8_t SCANNING_INTERVAL=30;
  ...
  // TODO: Instancier les capteurs de distance/calcul de longeurs
  ...
  // Longueur des murs
  //! a revoir avec l'algorithme
  uint32_t lengthLeft = 1234;
  uint32_t lengthRight = 4567;
        ...
        switch (PathPart) {
          ...
          case secondOrFourthTurn:
            ...
            if (turnCounter == 2) {
              ...
              LEFT_CAPTOR_ADJUST=-18;
              RIGHT_CAPTOR_ADJUST=53;
            }
          ...
          case transmit:
            ...
            lengthLeft= calculateLenght(leftData,nData);
            lengthLeft=lengthLeft*254/10000;

			// On calcule la langueur du mur droite
            lengthRight= calculateLenght(rightData,nData);
            lengthRight=lengthRight*254/10000;
            ...
            for (;;) {
              if (PIND & 0x04) {
                char longueur[10]={};
                intToString(lengthLeft,longueur);
                DEBUG_PRINT((longueur));
                intToString(lengthRight,longueur);
                DEBUG_PRINT((longueur));
                led.flashingGreen();
