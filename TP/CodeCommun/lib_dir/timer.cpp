...
void Timer::start(uint16_t duration){
    ...
    *TCCRnA |= (1 << COM1A1) | (1 << COM1A0);           // OC3A mis sur "compare match", output = high level (p.128)
    uint16_t multiplier;
    switch(clockDivider_){ //on set le multiplicateur == F_CPU/clockDivider / 100
        case 1: multiplier = 8000; 
            duration = duration * 10; 
            break;
        case 8: multiplier = 10000; break;
        case 64: multiplier = 12500; break;
        default:
        case 256: multiplier = 313; break;
        case 1024: multiplier = 78; break;
    }
    if(timerNumber_ == 1){                               //Dans le cas qu'on prend timer 1, certains registres sont geres differement, ici on modifie les registres que l'on touchait pas dans le constructeur
        ...
        //on divise la duree par 10 pour que les multiplicateurs soient corrects
        OCR1A =  multiplier * (duration /10);           //OCR1A est set ici car pour timer 1 il a 16 bit
        ...
        *OCRnA = multiplier * (duration /10);      //clockFrequence_ / 1000 = valeur equivalent a 1 ms, on multiplie par duration avant pour garder plus de precision
...
void Timer::setClockDivider(uint16_t divider){
    ...
    switch(divider){
        ...
        *TCCRnB &= ~(1 << CS12); // Diminution de l'horloge a 1/64 (p.131) (bit CS12 mit a 0)
        ...
        case 256: //CS1n = 100
        default: //by default clock is divided by 256
        clockDivider_ = 256; //correction du divider dans le cas qu'on execute le defaut
        *TCCRnB |=  (1 << CS12);
        *TCCRnB &= ~((1 << CS10) | (1 << CS11)); // Diminution de l'horloge a 1/256 (p.131) (bit CS10 et CS11 mit a 0)
        break;

        case 1024: //CS1n = 101
        *TCCRnB |=  (1 << CS10) | (1 << CS12);
        *TCCRnB &= ~(1 << CS11); // Diminution de l'horloge a 1/1024 (p.131) (bit CS10 mit a 0)
        break;  
    }
